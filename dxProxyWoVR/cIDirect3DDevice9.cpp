...
static bool drawCountsLogged = false;
static unsigned long sceneCount = 0;
...
{
	sceneCount++;
	if (sceneCount == 300) drawCountsLogged = false;
...
{
	static unsigned int textureSetCount = 0;
	static IDirect3DBaseTexture9* lastTexture = nullptr;
	IDirect3DSurface9* textureSurface = nullptr;
	D3DSURFACE_DESC textureDesc = {};
	if (pTexture)
	{
		IDirect3DTexture9* texture2D = nullptr;
		if (SUCCEEDED(pTexture->QueryInterface(__uuidof(IDirect3DTexture9), (void**)&texture2D)))
		{
			texture2D->GetSurfaceLevel(0, &textureSurface);
			texture2D->Release();
		}
		if (textureSurface)
		{
			textureSurface->GetDesc(&textureDesc);
			textureSurface->Release();
		}
	}
	if (pTexture != lastTexture)
	{
...
HRESULT cIDirect3DDevice9::DrawPrimitive(D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount)
{
	...
	m_pIDirect3DDevice9->GetTexture(0, &currentTexture);
	if (currentTexture)
	{
		IDirect3DTexture9* currentTexture2D = nullptr;
		if (SUCCEEDED(currentTexture->QueryInterface(__uuidof(IDirect3DTexture9), (void**)&currentTexture2D)))
		{
			currentTexture2D->GetSurfaceLevel(0, &currentSurface);
			currentTexture2D->Release();
		}
	}
	if (currentSurface)
	{
		D3DSURFACE_DESC drawTextureDesc = {};
		currentSurface->GetDesc(&drawTextureDesc);
...
HRESULT cIDirect3DDevice9::DrawIndexedPrimitive(
	...
	UINT primCount)
{
	return m_pIDirect3DDevice9->DrawIndexedPrimitive(
		PrimitiveType,
		BaseVertexIndex,
		MinVertexIndex,
		NumVertices,
...
HRESULT cIDirect3DDevice9::DrawPrimitiveUP(
	D3DPRIMITIVETYPE PrimitiveType,
	UINT PrimitiveCount,
	...
	UINT VertexStreamZeroStride)
{
	...
	return m_pIDirect3DDevice9->DrawPrimitiveUP(
		PrimitiveType,
		PrimitiveCount,
