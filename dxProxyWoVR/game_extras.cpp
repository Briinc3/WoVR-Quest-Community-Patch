...
void(__fastcall msub_4A8720)()
{
    ...
    if (svr->isEnabled())
    {
        ...
        if (SUCCEEDED(devDX9->CreateQuery(D3DQUERYTYPE_EVENT, &eventQuery)) && eventQuery)
        {
            eventQuery->Issue(D3DISSUE_END);

            while (S_FALSE == eventQuery->GetData(nullptr, 0, D3DGETDATA_FLUSH))
            {
            }
...
void RunFrameUpdateController()
{
    ...
    {
        originS.vector4_f32[0], originS.vector4_f32[1], originS.vector4_f32[2], 0.0f, 1.0f, 0.0f, 1.0f,
        end.vector4_f32[0],    end.vector4_f32[1],    end.vector4_f32[2],    0.0f, 1.0f, 0.0f, 1.0f
    };
...
void RunControllerGame()
{
    ...
    static bool rightTriggerBackpack = false;
    static bool leftTriggerL = false;
    static bool leftTriggerR = false;
    static int leftTriggerMode = 0;
    static bool mouseButtonDownL = false;
    static bool mouseButtonDownR = false;
    static bool lxRunning = false;
    static bool moveFwd = false;
    static bool moveBack = false;
    static bool moveStrafeL = false;
    static bool moveStrafeR = false;
    static float rightStickPitch = 0.0f;
    ...
    if (vr::VRInput()->UpdateActionState(&actionSet, setSize, setCount) == vr::VRInputError_None) {
        ...
        if (vr::VRInput()->GetAnalogActionData(input.game.rotate, &analogActionData, sizeof(analogActionData), vr::k_ulInvalidInputValueHandle) == vr::VRInputError_None && analogActionData.bActive == true) {
            ...
            if (gPlayerObj)
            {
                ...
                {
                    rightStickPitch += (analogActionData.y / 50.0f);

                    if (rightStickPitch > 1.2f)
                        rightStickPitch = 1.2f;
        ...
        if (vr::VRInput()->GetAnalogActionData(input.game.left_trigger, &analogActionData, sizeof(analogActionData), vr::k_ulInvalidInputValueHandle) == vr::VRInputError_None && analogActionData.bActive == true)
        {
            if (gPlayerObj)
            {
                if (analogActionData.x > 0.25f && leftTriggerMode == 0)
                {
                    if (bumperPressedL)
                    {
                        // Left Grip + Left Trigger = Hotbar Page Up
                        leftTriggerMode = 3;
                        setKeyDown((unsigned int)VK_UP);
                    }
                    else if (bumperPressedR)
                    {
                        // Right Grip + Left Trigger = Action Bar 11
                        leftTriggerMode = 2;
                        setKeyDown((unsigned int)VK_OEM_MINUS);
                    }
                    else
                    {
                        // Left Trigger = Action Bar 12
                        leftTriggerMode = 1;
                        setKeyDown((unsigned int)VK_OEM_PLUS);
                    }
                }
                else if (analogActionData.x <= 0.25f && leftTriggerMode != 0)
                {
                    if (leftTriggerMode == 3)
                    {
                        setKeyUp((unsigned int)VK_UP);
                    }
                    else if (leftTriggerMode == 2)
                    {
                        setKeyUp((unsigned int)VK_OEM_MINUS);
                    }
                    else if (leftTriggerMode == 1)
                    {
                        setKeyUp((unsigned int)VK_OEM_PLUS);
                    }
        ...
        if (vr::VRInput()->GetAnalogActionData(input.game.right_trigger, &analogActionData, sizeof(analogActionData), vr::k_ulInvalidInputValueHandle) == vr::VRInputError_None && analogActionData.bActive == true)
        {
            ...
            if (bumperPressedL)
            {
                if (analogActionData.x > 0.25f && rightTriggerBackpack == false)
                {
                    rightTriggerBackpack = true;
                    setKeyDown((unsigned int)'M');
                }
                else if (analogActionData.x <= 0.25f && rightTriggerBackpack == true)
                {
                    rightTriggerBackpack = false;
        ...
        if (vr::VRInput()->GetDigitalActionData(input.game.right_stick_click, &digitalActionData, sizeof(digitalActionData), vr::k_ulInvalidInputValueHandle) == vr::VRInputError_None && digitalActionData.bActive == true)
        {
            ...
            else
            {
               // Hotbar Button 2
    if (digitalActionData.bState == true && digitalActionData.bChanged == true)
        setKeyDown((unsigned int)'4');
    else if (digitalActionData.bState == false && digitalActionData.bChanged == true)
        setKeyUp((unsigned int)'4');
        ...
        if (vr::VRInput()->GetDigitalActionData(input.game.button_a, &digitalActionData, sizeof(digitalActionData), vr::k_ulInvalidInputValueHandle) == vr::VRInputError_None && digitalActionData.bActive == true)
        {
            if (gPlayerObj)
            {
                if (bumperPressedL)
                {
                    // Target Nearest Friendly Player - bind F8 in WoW
                    if (digitalActionData.bState == true && digitalActionData.bChanged == true)
                    {
                        setKeyDown((unsigned int)VK_F8);
                    }
                    else if (digitalActionData.bState == false && digitalActionData.bChanged == true)
                    {
                        setKeyUp((unsigned int)VK_F8);
                    }
                }
                else if (bumperPressedR)
                {
                    if (digitalActionData.bState == true && digitalActionData.bChanged == true)
                    {
                        setKeyDown((unsigned int)VK_TAB);
                    }
                    else if (digitalActionData.bState == false && digitalActionData.bChanged == true)
                    {
                        setKeyUp((unsigned int)VK_TAB);
                    }
                }
                else
                {
        ...
        if (vr::VRInput()->GetDigitalActionData(input.game.button_x, &digitalActionData, sizeof(digitalActionData), vr::k_ulInvalidInputValueHandle) == vr::VRInputError_None && digitalActionData.bActive == true)
        {
            if (gPlayerObj)
            {
                if (bumperPressed)
                {
                    ...
                    if (digitalActionData.bState == true && digitalActionData.bChanged == true)
                    {                      
                        setKeyDown((unsigned int)'5');
                    }
                    else if (digitalActionData.bState == false && digitalActionData.bChanged == true)
                        setKeyUp((unsigned int)'5');
                }
                else
                {
                    // Hotbar Button 6
                    if (digitalActionData.bState == true && digitalActionData.bChanged == true)
                        setKeyDown((unsigned int)'6');
                    else if (digitalActionData.bState == false && digitalActionData.bChanged == true)
                        setKeyUp((unsigned int)'6');
                }
            }
        }
        ...
        if (vr::VRInput()->GetDigitalActionData(input.game.button_y, &digitalActionData, sizeof(digitalActionData), vr::k_ulInvalidInputValueHandle) == vr::VRInputError_None && digitalActionData.bActive == true)
        {
            if (bumperPressed && gPlayerObj)
            {
                ...
                if (digitalActionData.bState == true && digitalActionData.bChanged == true)
                {                 
                    setKeyDown((unsigned int)'7');
                }
                else if (digitalActionData.bState == false && digitalActionData.bChanged == true)
                    setKeyUp((unsigned int)'7');
            }
            else
            {
                // Hotbar Button 8
        ...
        if ((cfg_hmdOnward & 2) == 2)
        {
            ...
            setVerticalRotation(-anglesPalm.vector4_f32[0] + rightStickPitch);
        }
        else
        {
            anglesPalm = GetAngles(matControllerPalm[0]);
            setVerticalRotation(-anglesPalm.vector4_f32[0] - 0.2f + rightStickPitch);
        }
        ...
        else if (rightStickActive)
        {
            setHorizontalRotation(
                gRotation + hRotationStickOffset,
                gCamRotation,
                mouseHold
            );
