...
+ Ability to switch back to the curved UI look if you wish. The current D3D9.dll file has the flat UI, while the D3D9_(curved UI).dll has the curved UI. To switch, all you need to do is remove the (Curved ui) text from that dll, and rename the current one to D3D9(Flat ui).dll and vise versa if you want to switch back.
